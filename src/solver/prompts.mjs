      ...
      solutions: {
        ...
        items: {
          ...
          properties: {
            ...
            code: { type: "string" },
            // Why: the browser lane captures no visible tests, so without these the first
            // fill is completely unvalidated and the only oracle is a Run Check round
            // trip we may not have time to pay for twice. Encoded as JSON text because
            // strict schema mode cannot express "any JSON value".
            examples: {
              type: "array",
              minItems: 2,
              maxItems: 4,
              items: {
                ...
                required: ["argsJson", "expectedJson"],
              },
            },
          },
          required: ["id", "code", "examples"],
        },
      },
    },
    required: ["solutions"],
...
export function javascriptRequest(tasks, feedback = {}) {
  return {
    instructions: [
      ...
      "For each solution also return 2-4 examples you are confident the prompt requires, as argsJson (a JSON array of the call arguments) and expectedJson (the JSON return value).",
      "Prefer examples copied verbatim from the prompt; add a plain edge case only when the prompt states one. Never invent an example you are unsure of.",
