...
import { recordLatencyObservation } from "../src/runtime/latency-store.mjs";
import { solveManifest } from "../src/solver/orchestrator.mjs";
import { browserLatencyKey } from "../src/browser/runner.mjs";
...
try {
  ...
  const latency = await recordLatencyObservation(
    path.join(projectRoot, ".cache", "latency-observations.json"),
    {
      key: browserLatencyKey(runtime.config),
      elapsedMs: summary.wallTimeMs,
      taskCount: report.run.taskCount,
    },
  );
  summary.latencyObservation = latency;
